...
| **Form 8937** | `—` for IAUI, NIHI, XBCI, XQQI and XSPI | Those five 8937 tabs are empty — NEOS has not filed one for them yet |
| **12-Month Trailing Distribution Rate** | `—` for IAUI, MLPI, NEHI, NIHI, NLSI, SPYH, XBCI, XQQI and XSPI | Their Distribution Information block publishes the Distribution Rate and the 30-Day SEC Yield only; the trailing row is absent even where the fund is older than 12 months (IAUI), so it is never inferred from the payout calendar |
| **SEDOL / FIGI** | always `—` | NEOS publishes neither (the holdings CSV carries a CUSIP, and the fund page CUSIP/ISIN), so the app shows the documented data limitation |
